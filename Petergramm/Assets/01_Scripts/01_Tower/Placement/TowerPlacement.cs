...
namespace _01_Scripts._01_Tower.Placement
{
   public class TowerPlacement : MonoBehaviour
   {
      ...
      [SerializeField] private List<TowerRuntime> towerPrefab = new();
      ...
      private void RayForTowerPosition()
      {
         using (TowerPlacementRayCast.Auto())
         {
            ...
            if (Physics.Raycast(ray, out RaycastHit hitDebug, 100f)) debugButtons.TestLightFollowMouse(hitDebug.point);
      ...
      private void DespawnTower()
      {
         ...
         TowerPrice(_draggingTower.CurrentStats.costs);
         Destroy(_draggingTower.gameObject);
         _draggingTower = null;
         _isDragging = false;
      }
      ...
      private void DestroyTower(Vector3Int gridCoord)
      {
         using (TowerPlacementDestroyTower.Auto())
         {
            ...
            var occupant = placementCoords.occupant;
            TowerPrice(occupant.CurrentStats.costs);
      ...
      private void TowerPrice(float price) { TowerPlacedOrRemoved?.Invoke(price); }
   }
}