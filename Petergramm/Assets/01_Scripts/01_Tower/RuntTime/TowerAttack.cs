...
namespace _01_Scripts._01_Tower.RuntTime
{
   public class TowerAttack : MonoBehaviour
   {
      ...
      private static readonly ProfilerMarker LookMarker = new ProfilerMarker("Tower_LookAtEnemy");
      private static readonly ProfilerMarker NewTargetMarker = new ProfilerMarker("Tower_NewTarget");
      private static readonly ProfilerMarker FireMarker = new ProfilerMarker("Tower_Fire");
      private static readonly ProfilerMarker SpawnProjMarker = new ProfilerMarker("Tower_SpawnProj");
      ...
      private void Update()
      {
         ...
         using (LookMarker.Auto()) LookAtEnemy();
         if (attacksPerSecond <= 0f) return;
         var attackInterval = 1f * Random.Range(0.9f, 1.1f) / attacksPerSecond;

         if (!(this._cd <= 0f)) return;

         this._cd = attackInterval;
         using (NewTargetMarker.Auto()) { FindNextTarget(); }

         this.Fire();
      }

      private void ApplyStatsToProjectiles
      (
         TowerAttackSO attackData,
         TowerStats stats,
         ProjectileRuntime projectileRuntime,
         TowerEffectValues effectValues
      )
      {
